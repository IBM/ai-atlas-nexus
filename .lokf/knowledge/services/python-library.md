...
Functional building blocks live under `src/ai_atlas_nexus/blocks/`: `graph_explorer` (including a Pyoxigraph explorer), `risk_detector`, `risk_explorer`, `risk_categorization`, `risk_mapping`, `shacl`, `hf_data_loader`, prompt building, and an `inference` package with engines for IBM watsonx.ai (WML), Ollama, vLLM, RITS, AWS Bedrock, OpenAI-compatible servers, and Hugging Face.

Requires Python `>=3.11, <3.14.4` (see `pyproject.toml`). LLM inference backends are optional extras: `[wml]`, `[ollama]`, `[vllm]`, `[bedrock]`. The Bedrock engine (`blocks/inference/bedrock.py`, extra `boto3`/`botocore`) is present in the code but not yet listed in the README's supported-API section.
