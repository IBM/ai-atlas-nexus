...
derivedFrom:
  ...
  - https://w3id.org/ai-atlas-nexus/knowledge/references/owasp-agentic-skills-top10
---

# Overview

The **knowledge graph** lives as LinkML instance data in `src/ai_atlas_nexus/data/knowledge_graph/` - one YAML file per curated source, e.g. `risk_atlas_data.yaml` (IBM AI Risk Atlas), `nist_ai_rmf_data.yaml` and `nist_ai_rmf_actions_data.yaml` (NIST AI RMF), `mit_ai_risk_repository_data.yaml` plus mitigation/controls files (MIT), `owasp_llm_2.0_data.yaml`, `owasp_asi_data.yaml` and `owasp_ast10_data.yaml` (OWASP), `air_2024_data.yaml`, `ailuminate.yaml`, `credo.yaml`, `granite_guardian_dimensions.yaml`, ethics principles (`principles_*.yaml`), AI models (`ibm_granite_*` including Granite 4.2, `shieldgemma_*`), evaluations (`ai_eval_data.yaml`), and shared definitions (`ai_commons_data.yaml`).
